package com.Neko.YouXi.HotUpdate;
...
public class ClassLoaderHookHelper {
    ...
    private static Object[] filterElements(Object[] elements, String packagePrefix) {
        List<Object> filtered = new ArrayList<>();
        
        try {
            for (Object element : elements) {
                // 检查这个Element是否包含我们的包
                if (containsOurPackage(element, packagePrefix)) {
...
private static boolean containsOurPackage(Object element, String packageName) {
        try {
            // 通过反射获取dexFile
            ...
            // 获取类名列表
            Method getClassNameListMethod = dexFile.getClass().getDeclaredMethod("getClassNameList");
            String[] classNames = (String[]) getClassNameListMethod.invoke(dexFile);
            
            // 检查是否有我们的包下的类
            for (String className : classNames) {
                // className格式: com/raincat/ClassName
                if (className.startsWith(packageName + "/")) {
                    return true;
                }
            }
        } catch (Exception e) {
            // 反射失败，返回true以兼容（相当于不过滤）
            e.printStackTrace();
            return true;
        }
        return false;
